...
-------------------------------------------------------------------------------
 ...
 The considered GrandOrgue ODF syntax is :
    [object_uid]  ; comment, section name (object_uid) can contain only alphanumeric characters
 ...
 Versions history :
   ...
   v2.7 - 07 Nov. 2023  - manage in the viewer the pipe borrowing format REF:xx:xx:xx + minor improvements
                          ...
                          several minor improvements and bugs fixing
   v2.8 - 13 Nov. 2023  - improvement of the section name drag&drop visual behavior
                          drag&drop permits to reorder between them sections of the same type
                          removal of the menu option asking to auto expand sections tree on section selection. Double clicking on an item of sections list makes expand the sections tree to show this section.
...
APP_VERSION = 'v2.8'
RELEASE_DATE = 'November 18th 2023'
...
class C_ODF_DATA_CHECK:
    ...
    def check_odf_data(self, progress_status_update_fct):
        ...
        if logs.nb_get() <= 3:  # there are 3 log lines when no error : check start message + detected panel format + number of checked attributes
            logs.add("None error found, can be loaded in GrandOrgue for a complete check")
...
class C_ODF_DATA(C_ODF_DATA_CHECK, C_ODF_MISC):
    ...
    def object_link(self, object_uid, kinship_objects_list, relationship):
        ...
        if relationship == TO_PARENT:
            ...
            if object_type == 'PanelElement' and len(kinship_objects_list) > 1:
                ...
                messagebox.showerror(title='Selection error', message=f'A {object_type} section can have only one child section.')
                return None
        ...
        if parent_object_type == 'WindchestGroup' and child_object_type in ('Rank', 'Stop'):
            ...
            if operation == 'link' and parent_object_uid != prev_parent_panel_uid:
                ...
                if child_object_type == 'PanelElement':
                    ...
                    logs.add(f"{prev_parent_panel_uid} : attribute NumberOfGUIElements updated to {nb}")
                else:
                    ...
                    logs.add(f"{prev_parent_panel_uid} : attribute NumberOfImages updated to {nb}")
    ...
    def object_children_ref_update(self, object_uid, ref_object_type, operation=None, ref_object_uid=None):
        # update in the given object the arrangement (sorted) of the references to the given object type
        # update the NumberOfxxxx attribute value
        ...
        # define the name of the attribute in the given object which contains the number of referenced objects of the given type
        if object_type in ('Coupler', 'DivisionalCoupler', 'Stop', 'Switch', 'Tremulant') and ref_object_type == 'Switch':
        ...
        # define the name of the attributes in the given object which are referencing objects of the given type
        if object_type == 'General':
            # in a General object, there are two referencing attributes : the referenced object ID and the manual ID
            object_type_attr_name = ref_object_type + 'Number'
            if ref_object_type in ('Coupler', 'Stop'):
                object_type_attr_name2 = ref_object_type + 'Manual'
            else:
                object_type_attr_name2 = None
        else:
            object_type_attr_name = ref_object_type
            object_type_attr_name2 = None

        # make the list of the references to objects of the given type which are defined in the given object
        # and delete in the given object the lines having these references
        references_dic = {}        # dictionary with as keys the references and as value a tuple : (pre-fix, comment)
        nb_of_attr_line_nb = None  # line number in the given object where is located the NumberOf attribute
        first_ref_line_nb = None   # line number in the given object where is located the first reference
        line_nb = 0
        nb_of_attr_value = None
        object_lines_list = object_dic['lines']
        while line_nb < len(object_lines_list):
            # scan the lines of the given object
            if object_lines_list[line_nb].startswith(ref_object_type):
                # the current line should contain a reference to the given object type
                (error_msg, attr_name, attr_value, comment) = self.object_line_split(object_lines_list[line_nb])
                if attr_name != None and attr_value.lstrip("-+").isdigit():
                    if attr_name[:-3] == object_type_attr_name:
                        # the attribute is referencing an object of the given type
                        # if the referencing value has a plus/minus prefix (used in General and Divisional), store it and remove it from the attribute value
                        if attr_value[0] in ('+', '-'):
                            attr_prefix = attr_value[0]
                            attr_value = attr_value[1:]
                        else:
                            attr_prefix = ''
                        # store the reference of the object and the comment if it is not a reference to remove
                        if not(operation == 'remove' and int(attr_value) == ref_object_id):
                            references_dic[int(attr_value)] = (attr_prefix, comment)
                        # store the line number if it is the first referencing line of the object type
                        if first_ref_line_nb == None:
                            first_ref_line_nb = line_nb
                        # delete the referencing line
                        del object_lines_list[line_nb]
                        line_nb -= 1
                    elif attr_name[:-3] == object_type_attr_name2:
                        # the attribute is referencing the manual ID in which is acting the referenced object
                        # no need to store its value, it can be found in the ODF
                        # delete the referencing line
                        del object_lines_list[line_nb]
                        line_nb -= 1

            if object_lines_list[line_nb].startswith(nb_of_attr_name + '='):
                ...
                nb_of_attr_value = int(object_lines_list[line_nb][len(nb_of_attr_name + '='):])
            line_nb += 1

        # add if needed the NumberOfxxxx attribute
##        if nb_of_attr_line_nb == None:
##            # attribute NumberOfxxxx or SwitchCount not found : add it in the last line of the object
##            object_lines_list.append(nb_of_attr_name + '=0')
##            nb_of_attr_line_nb = len(object_lines_list) - 1
        # update if needed the position to which write the first reference
##        if first_ref_line_nb == None or first_ref_line_nb < nb_of_attr_line_nb + 1:
##            first_ref_line_nb = nb_of_attr_line_nb + 1

        if operation == 'add':
            references_dic[int(ref_object_id)] = ('', '')  # no prefix, no comment

        # update the NumberOfxxxx attribute if it exists
        if nb_of_attr_value != None:
            nb_of_value = len(references_dic)
            if nb_of_attr_value != nb_of_value:
        ...
        # insert all the references in the given object
        insert_idx = first_ref_line_nb
        ref_idx = 1
        for ref_obj_id, (prefix, comment) in sorted(references_dic.items()):
            line = self.object_line_join(object_type_attr_name + str(ref_idx).zfill(3), prefix + str(ref_obj_id).zfill(3), comment)
            object_lines_list.insert(insert_idx, line)
            insert_idx += 1
            if object_type_attr_name2 != None:
                ...
                manual_uid = self.object_parent_manual_get(ref_object_type + str(ref_obj_id).zfill(3))
                if manual_uid != None:
                    # add a line with the reference to the Manual
                    line = self.object_line_join(object_type_attr_name2 + str(ref_idx).zfill(3), manual_uid[-3:])
                else:
                    line = self.object_line_join(object_type_attr_name2 + str(ref_idx).zfill(3), '')
                object_lines_list.insert(insert_idx, line)
                insert_idx += 1
            ref_idx += 1
    ...
    def object_poss_children_type_list_get(self, object_uid):
        ...
        elif object_type == 'General': child_types_list = ['Coupler', 'DivisionalCoupler', 'Stop', 'Switch', 'Tremulant']
...
##        self.general_menu.add_command(label="Extend manual's compass...", command=self.manual_compass_extend)
    ...
    def object_b1_motion(self, event=None):
        ...
        if self.object_dragging_in_progress and (self.drag_overflown_object_uid != overflown_object_uid or event == None) and self.selected_widget_b4 != self.lst_links_list:
            ...
            if overflown_object_uid != None and self.dragged_object_uid != None:
                if overflown_object_uid[:-3] == self.dragged_object_uid[:-3] and overflown_object_uid != self.dragged_object_uid:
                    # dragged and overflown objects UID have the same type and not the same UID
                    ...
                    if (self.dragged_object_type in self.odf_data.object_poss_children_type_list_get(overflown_object_uid) and
                        self.dragged_object_type in ('Coupler', 'Enclosure', 'PanelElement', 'PanelImage', 'Rank', 'Switch', 'Stop', 'Tremulant') and
                        ...
                        if not self.key_control_pressed_bool:
                            ...
                            if overflown_object_uid not in self.dragged_object_parents_list:
                                ...
                                self.wnd_main['cursor'] = 'sb_up_arrow'  #top_side'
                        else:
